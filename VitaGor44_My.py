...
class Training:
    ...
    def show_training_info(self, training_type, calories) -> InfoMessage:
        ...
        info = InfoMessage(training_type, self.duration, self.get_distance(), self.get_mean_speed(), calories)
        return info
...
class Running(Training):
    ...
    def get_spent_calories(self) -> float:
        ...
        calories_Running = (coeff_calorie_1 * self.get_mean_speed() - coeff_calorie_2) * self.weight / \
...
class SportsWalking(Training):
    ...
    def get_spent_calories(self) -> float:
        """Расчет (0.035 * вес + (средняя_скорость**2 // рост) * 0.029 * вес) * время_тренировки_в_минутах)"""

        coeff_calorie_3 = 0.035
        coeff_calorie_4 = 0.029

        calories_SportsWalking = (coeff_calorie_3 * self.weight + (self.get_mean_speed()**2 // self.height) *
...
class Swimming(Training):
    ...
    def get_mean_speed(self) -> float:
        ...
        длина_бассейна * count_pool / M_IN_KM / время_тренировки.
         """
        mean_speed_Swimming = self.length_pool * self.count_pool / Training.M_IN_KM / (self.duration * 60)
    ...
    def get_spent_calories(self) -> float:
        ...
        calories_Swimming = (self.get_mean_speed() + coeff_calorie_5) * 2 * self.weight
...
def main(training: Training) -> None:
    """Главная функция."""
    info = training.show_training_info(training.__doc__, training.get_spent_calories())
